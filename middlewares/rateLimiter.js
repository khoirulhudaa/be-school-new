...
// 1. Global limiter — pakai Redis store
const globalLimiter = rateLimit({
    windowMs: 15 * 60 * 1000,
    limit: 500,
    standardHeaders: 'draft-8',
    ...
    message: { success: false, message: 'Terlalu banyak permintaan, coba lagi nanti.' },
    statusCode: 429,
    skip: (req) => req.path.startsWith('/socket.io'), // ← skip socket
});

// 2. Strict limiter — pakai Redis store
const strictLimiter = rateLimit({
    windowMs: 60 * 1000,
    limit: 5,
    store: new RedisStore({
    ...
    keyGenerator: (req) => {
        const profile = req.user?.profile || req.user;
        if (profile?.id) return `scan:${profile.id}`;
        return req.ip;
    },
    message: { success: false, message: 'Terlalu banyak percobaan, tunggu 1 menit.' },
    statusCode: 429,
